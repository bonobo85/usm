...
function Inner() {
  ...
  const create = async () => {
    ...
    const r = await api("sanction:create_ticket", form);
    setSaving(false);
  ...
  return (
    <div>
      ...
      <Tabs tabs={[
        { label: `Mes (${mine.length})`, content: <div className="grid sm:grid-cols-2 gap-3">{mine.map(Card)}{mine.length === 0 && <p className="text-sm text-[var(--texte-muted)]">Aucun.</p>}</div> },
        ...
        { label: `Tous (${tickets.length})`, content: <div className="grid sm:grid-cols-2 gap-3">{tickets.map(Card)}{tickets.length === 0 && <p className="text-sm text-[var(--texte-muted)]">Aucun.</p>}</div> }
      ]} />
      <Modal open={open} onClose={() => setOpen(false)} title="Nouveau ticket">
        <div className="space-y-4">
          <div><label className="label">Type</label><select className="input" value={form.type} onChange={e => setForm({...form, type: e.target.value})}><option value="retour">Retour</option><option value="sanction">Sanction</option></select></div>
          ...
          <div><label className="label">Contenu</label><textarea className="input" rows={3} value={form.contenu} onChange={e => setForm({...form, contenu: e.target.value})} /></div>
          <div><label className="label">Priorité</label><select className="input" value={form.priorite} onChange={e => setForm({...form, priorite: e.target.value})}><option value="basse">Basse</option><option value="normale">Normale</option><option value="haute">Haute</option><option value="critique">Critique</option></select></div>
          {form.type === "sanction" && (
          ...
          <div className="flex justify-end gap-2"><button className="bouton-gris" onClick={() => setOpen(false)}>Annuler</button><button className="bouton-bleu" onClick={create} disabled={!form.titre.trim() || saving}>{saving ? "..." : "Créer"}</button></div>
        </div>
      </Modal>
    </div>
  );
