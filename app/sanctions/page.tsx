...
function Inner() {
  ...
  const Card = (t: any) => (
    <div key={t.id} className="carte">
      <div className="flex justify-between mb-1">
        ...
        <span className="text-xs px-2 py-0.5 rounded bg-[var(--bleu)]">{t.statut}</span>
      </div>
  ...
  return (
    <div>
      ...
      <Tabs tabs={[
        { label: "Mes tickets", content: <div className="grid sm:grid-cols-2 gap-3">{mine.map(Card)}</div> },
        ...(rang >= 7 ? [{ label: "À traiter", content: <div className="grid sm:grid-cols-2 gap-3">{traiter.map(Card)}</div> }] : []),
        { label: "Tous", content: <div className="grid sm:grid-cols-2 gap-3">{tickets.map(Card)}</div> }
      ]} />
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Nouveau ticket">
        <div className="space-y-3">
          ...
          <div><label className="label">Titre</label><input className="input" value={form.titre} onChange={e => setForm({...form, titre: e.target.value})} /></div>
          ...
          <div className="flex justify-end gap-2"><button className="bouton-gris" onClick={() => setOpen(false)}>Annuler</button><button className="bouton-bleu" onClick={create}>Créer</button></div>
        </div>
      </Modal>
    </div>
  );
