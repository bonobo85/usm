...
function Inner() {
  ...
  const create = async () => {
    if (!form.candidat_nom || !form.date_rc || !me?.id) return;
    setSaving(true);
    await supa.from("recrutements").insert({ ...form, createur_id: me.id, statut: "planifie" });
    setSaving(false);
    setOpenRC(false);
    setStep(1);
  ...
  const submitResult = async () => {
    if (!openResult || !me?.id) return;
    setSaving(true);
    const notes = [
      parseFloat(resultForm.tir_note) || 0, parseFloat(resultForm.conduite_note) || 0,
      parseFloat(resultForm.procedure_note) || 0, parseFloat(resultForm.comportement_note) || 0
    ];
    ...
    await supa.from("rc_resultats").insert({
      ...
      note_globale: globale, points_forts: resultForm.points_forts, points_faibles: resultForm.points_faibles,
      observations: resultForm.observations, resultat: resultForm.resultat, redacteur_id: me.id
    });
  ...
  return (
    <div>
      ...
      <Tabs tabs={[
        {
          label: `RC à faire (${rcPending.length})`,
          content: (
              ...
              {rcPending.map(r => (
                <div key={r.id} className="carte">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded text-white ${r.statut === "planifie" ? "bg-[var(--bleu)]" : "bg-[var(--or)]"}`}>
                      {r.statut === "planifie" ? "Planifié" : "En cours"}
                    </span>
                  </div>
                  ...
                  <div key={r.id} className="carte">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded text-white ${r.resultat === "admis" ? "bg-[#2D8B4E]" : r.resultat === "refuse" ? "bg-[var(--rouge)]" : "bg-[var(--or)]"}`}>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between">
              ...
              <div className="flex gap-2">
                <button className="bouton-gris" onClick={() => { setOpenRC(false); setStep(1); }}>Annuler</button>
                <button className="bouton-or" onClick={create} disabled={!form.date_rc || saving}>{saving ? "Création..." : "Créer le RC"}</button>
              </div>
            </div>
          </div>
        )}
      ...
      {/* Result modal */}
      <Modal open={!!openResult} onClose={() => setOpenResult(null)} title={`Résultat — ${openResult?.candidat_nom}`} size="lg"
        footer={<>
          <button className="bouton-gris" onClick={() => setOpenResult(null)}>Annuler</button>
          <button className="bouton-or" onClick={submitResult} disabled={saving}>{saving ? "Validation..." : "Valider"}</button>
        </>}
      >
        <div className="space-y-4">
